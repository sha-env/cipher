...
# --- LOGIKA CAESAR CIPHER (DIAMBIL DARI KODE ANDA) ---

ALPH_LO = string.ascii_lowercase
ALPH_UP = string.ascii_uppercase

# Frekuensi huruf bahasa Inggris (untuk frequency analysis)
# Catatan: Untuk teks Bahasa Indonesia, tabel ini sebaiknya diganti
# dengan distribusi huruf Bahasa Indonesia yang sesuai agar lebih akurat.
EN_FREQ = {
...
def brute_force_crack(ciphertext):
    ...
    candidates = []
    # Shift 0 adalah teks asli (tidak digeser), jadi kita mulai dari 0 untuk kelengkapan
    for s in range(0, 26):
...
def score_text_by_frequency(text, freq_table=EN_FREQ):
    ...
    # Normalize score by length to prevent very long texts from always having a higher score
...
def frequency_analysis_crack(ciphertext):
    ...
    # Urutkan berdasarkan score (skor kecil = lebih mirip bahasa alami)
...
class CaesarCipherApp(tk.Tk):
    def __init__(self):
        ...
        self.style.configure('TNotebook.Tab', padding=[10, 5], font=('Inter', 10, 'bold'))
